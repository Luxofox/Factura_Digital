...
public class ControllerFactura implements Serializable {
    ...
    public void addListaCarrito() throws Exception {
        ...
        setTotal(total + subtotal + ((item.getCantidad() * (item.getProducto().getPrecio() * getImpuestoVenta()) / 100)) - ((item.getCantidad() * (item.getProducto().getPrecio() * item.getDescuento()) / 100)));
    ...
    public void deleteListaCarrito(Item item) throws Exception {
        setImpuestoVenta(0);
        setTotal(total - subtotal - ((item.getCantidad() * (item.getProducto().getPrecio() * item.getProducto().getImpuestoVenta()) / 100)) - ((item.getCantidad() * (item.getProducto().getPrecio() * item.getDescuento()) / 100)));
        setSubtotal(subtotal - (item.getProducto().getPrecio() * item.getCantidad()));
        carrito.remove(item);
    }
